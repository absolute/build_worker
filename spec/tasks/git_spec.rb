...
describe "git", "#new project" do  
  before (:all) do
      @drive_dir = "/Users/muthu/project/git_unitest"
  ...
  context "checkout on an empty drive" do     
    ...
    it "should end successfully" do  
      system(%{rake git:checkout DRIVE_DIR=#{@drive_dir} PROJECT_NAME=#{@project_name} PROJECT_URI=#{@project_uri} 2>error.log 1>out.log})    
      $?.success?.should == true
    end
    it "should checkout from repository" do            
      Dir.entries("#{@drive_dir}").select {|n| n =~ /^\w/}.should be_empty
      system(%{rake git:checkout DRIVE_DIR=#{@drive_dir} PROJECT_NAME=#{@project_name} PROJECT_URI=#{@project_uri}})    
...
describe "git", "#existing project" do
  ...
  context "update on an empty drive" do   
    ...
    it "should fail" do
      system(%{rake git:update DRIVE_DIR=#{@drive_dir} PROJECT_NAME=#{@project_name} 2>error.log 1>out.log})
      $?.success?.should == false 
    end                           
  ...
  context "update on a NON-empty drive" do
    ...
    context "with same project in drive" do
      it "should be successfull" do
        system(%{rake git:update DRIVE_DIR=#{@drive_dir} PROJECT_NAME=#{@project_name} BUILD_DIR=#{@build_dir} COMMIT_REPORT=#{@commit_report} 2>error.log 1>out.log})  
        $?.success?.should == true
      end
      it "should return commit id" do
        system(%{rake git:update DRIVE_DIR=#{@drive_dir} PROJECT_NAME=#{@project_name} BUILD_DIR=#{@build_dir} COMMIT_REPORT=#{@commit_report}})  
